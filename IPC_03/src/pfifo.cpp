...
void init_pfifo(PriorityFIFO *pfifo)
{
   ...
   pfifo->mutex = PTHREAD_MUTEX_INITIALIZER;
...
int empty_pfifo(PriorityFIFO *pfifo)
{
   ...
   // a false value indicates a program error
   return pfifo->cnt == 0;
}
...
void insert_pfifo(PriorityFIFO *pfifo, uint32_t id, uint32_t priority)
{
   ...
   {
      cond_wait(&pfifo->isNotFull, &pfifo->mutex);
   }
   require(pfifo != NULL, "NULL pointer to FIFO");                              // a false value indicates a program error
   require((id <= MAX_ID || id == SIGNAL_TERMINATE), "invalid id");                // a false value indicates a program error
...
uint32_t retrieve_pfifo(PriorityFIFO *pfifo)
{
   ...
   {
      cond_wait(&pfifo->isNotEmpty, &pfifo->mutex);
   }
